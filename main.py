...
from algorithm import Sim, Agent
...
value_matrix = np.zeros((agent.sim.n_batch, agent.sim.n_states), dtype=np.float)
im = plt.imshow(value_matrix, vmin=0, vmax=1, cmap='Oranges', animated=True)
im.set_zorder(0)
states = np.random.choice(agent.sim.n_states) * np.ones(agent.sim.n_states, dtype=int)
next_states = states
